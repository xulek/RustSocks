...
**Expected:** <40ms latency, >1200 conn/s
...
**Expected:** <70ms latency, >900 conn/s
...
**Result Interpretation:**
- High throughput (>1300 conn/s) indicates excellent connection handling
...
**Config Requirements:** Any
**Expected:** >500MB/s on native Linux (virtualized environments may plateau earlier)
...
- WSL2/virtualized localhost: typically 50-100 MB/s due to virtualization limits
- Native Linux/bare metal: >500 MB/s is attainable with loopback testing
...
**Config Requirements:** Any
**Expected:** >400MB/s sustained on native Linux
...
- Check handshake latency separately - should stay within <40ms minimal baseline
...
**Expected:** >1100 sessions/sec write throughput
...
- **Target:** >99% success rate (≥990/1000 successful)
...
- **Target:** >98% success rate (≥4900/5000 successful)
...
**Features:**
- Automatically starts the echo server
- Launches RustSocks in minimal mode (`config/rustsocks_minimal.toml` disables ACL/QoS/sessions and sets logging to `warn`)
- Captures logs under `loadtests/results/`
- Compares metrics against targets and prints the PASS/FAIL summary
...
|-----------|--------|------------|------|
| Pure SOCKS5 | <40ms | <60ms | >60ms |
| Full Pipeline | <70ms | <100ms | >100ms |
| ACL Evaluation | <3ms | <5ms | >5ms |
| Session Tracking | <5ms | <10ms | >10ms |
| Pool Reuse | <5ms | <8ms | >8ms |

### Throughput Targets

| Test | Target | Acceptable | Poor |
|------|--------|------------|------|
| Handshake-Only | >1200 conn/s | >800 conn/s | <800 conn/s |
| Full Pipeline | >900 conn/s | >600 conn/s | <600 conn/s |
| Data Transfer | >500 MB/s | >250 MB/s | <250 MB/s |
| DB Writes | >1100 sess/s | >700 sess/s | <700 sess/s |

### Stability Targets

| Test | Target | Acceptable | Poor |
|------|--------|------------|------|
| Concurrent 1000 | >99% success | >98% | <98% |
| Concurrent 5000 | >98% success | >96% | <96% |
...
|----------|---------|--------|--------|----------|
| minimal-pipeline | Baseline SOCKS5 | <40ms avg | Features OFF | 30s |
| full-pipeline | Production config | <70ms avg | Features ON | 30s |
| handshake-only | Connection throughput | >1200/s | Any | 30s |
| data-transfer | Bandwidth | >500MB/s | Any | 30s |
| large-transfer | Large files | >400MB/s | Any | 30s |
| acl-evaluation | ACL performance | <5ms | ACL ON | 30s |
| pool-efficiency | Pool effectiveness | 30-50% faster | Pool ON | 30s |
| session-churn | DB throughput | >1100/s | Sessions ON | 30s |
| long-lived | Stability | 0% drops | Any | 60s+ |
| concurrent-1000 | Concurrency | >99% success | Any | Auto |
| concurrent-5000 | High concurrency | >98% success | Any | Auto |
