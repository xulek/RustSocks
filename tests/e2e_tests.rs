...
async fn e2e_acl_block() {
    ...
    assert_eq!(rejected_sessions[0].user.as_ref(), "anonymous");
...
async fn e2e_session_tracking() {
    ...
    let expected_ip = echo_addr.ip().to_string();
    assert_eq!(session.user.as_ref(), "anonymous");
    assert_eq!(session.dest_ip.as_ref(), expected_ip.as_str());
...
async fn e2e_complete_flow() {
    ...
    let session = &active_sessions[0];
    let expected_ip = echo_addr.ip().to_string();
    assert_eq!(session.user.as_ref(), "testuser");
    assert_eq!(session.dest_ip.as_ref(), expected_ip.as_str());
    assert_eq!(session.dest_port, echo_addr.port());
