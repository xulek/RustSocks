...
impl TestMetrics {
    ...
    fn print_summary(&self, test_name: &str, elapsed: Duration) {
        ...
        let total_transfer_bytes = bytes_sent.saturating_add(bytes_recv);
        if total_transfer_bytes > 0 && elapsed.as_secs_f64() > 0.0 {
            let aggregate_bandwidth =
                total_transfer_bytes as f64 / elapsed.as_secs_f64() / 1_048_576.0;
            println!("  Aggregate Bandwidth:    {:.2} MB/s", aggregate_bandwidth);
        }
        println!("{}", "=".repeat(80));
    }
}
    ...
    for _ in 0..50 {
        ...
        tasks.push(tokio::spawn(async move {
            ...
            while Instant::now() < end_time {
                ...
                match result {
                    Ok(Ok((mut stream, _tcp_duration, socks5_duration))) => {
                        // Send substantial data to measure throughput (~512 KB per connection)
                        const CHUNKS_PER_CONNECTION: usize = 8;
                        let mut bytes_sent = 0u64;
                        let mut bytes_received = 0u64;

                        'chunks: for _ in 0..CHUNKS_PER_CONNECTION {
                            ...
                            while received < payload.len() {
                                match stream.read(&mut recv_buf[received..payload.len()]).await {
                                    Ok(0) => {
                                        break 'chunks;
                                    }
                                    Ok(n) => {
