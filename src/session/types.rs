...
///
/// Performance optimization: Uses Arc<str> for user, dest_ip, acl_decision, and acl_rule_matched
/// to enable cheap cloning (atomic counter increment instead of string copy).
...
pub struct Session {
    ...
    pub acl_rule_matched: Option<Arc<str>>,
    #[serde(
        serialize_with = "serialize_arc_str",
        deserialize_with = "deserialize_arc_str"
    )]
    pub acl_decision: Arc<str>,
}
...
{
    match opt {
        Some(arc) => serializer.serialize_some(&**arc),
...
impl Session {
    ...
    pub fn new(
        ...
        Self {
            ...
            close_reason: None,
            acl_rule_matched: acl_rule_matched.map(Arc::from),
            acl_decision: Arc::from(acl_decision.into()),
        }
    }
