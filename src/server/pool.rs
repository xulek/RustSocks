...
impl ConnectionPool {
    ...
    async fn emit_telemetry_event(
        ...
        if let Some(history) = self.telemetry.as_ref() {
            history
                .record_event(severity, category, message, details)
                .await;
        }
    }

    async fn log_capacity_events(
        &self,
        context: &str,
        ...
        if let Some(evicted) = evicted_addr {
            let message = format!(
                "Evicted oldest idle connection to {} while {} (global limit: {})",
                evicted, context, self.config.max_total_idle
            );
            let details = json!({
                "evicted_destination": evicted.to_string(),
                "max_total_idle": self.config.max_total_idle,
                "context": context,
            });
            self.emit_telemetry_event(
                TelemetrySeverity::Warning,
    ...
    /// Create a connection pool that can emit telemetry events.
    ...
    pub async fn get(&self, addr: SocketAddr) -> std::io::Result<TcpStream> {
        ...
        if let Err(ref err) = result {
            let message = format!("Failed to connect to {}: {}", addr, err);
            let details = json!({
    ...
    async fn connection_timeout_works() {
        ...
        if let Err(e) = result {
            assert!(
                matches!(
                    e.kind(),
                    std::io::ErrorKind::TimedOut | std::io::ErrorKind::NetworkUnreachable
                ),
                "expected timeout or unreachable error, got {:?}",
                e.kind()
            );
        }
    }
