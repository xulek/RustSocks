...
pub struct HtbQos {
    ...
    /// Per-user buckets keyed by shared Arc<str> to avoid cloning per request
    user_buckets: Arc<DashMap<UserKey, Arc<UserBucket>>>,
...
impl HtbQos {
    ...
    pub async fn allocate_bandwidth(&self, user: &str, bytes: u64) -> Result<()> {
        self.allocate_bandwidth_impl(user, || self.get_or_create_user_bucket_str(user), bytes)
            .await
    }

    pub async fn allocate_bandwidth_arc(&self, user: &Arc<str>, bytes: u64) -> Result<()> {
        let label = user.as_ref();
        self.allocate_bandwidth_impl(label, || self.get_or_create_user_bucket_arc(user), bytes)
            .await
    }

    async fn allocate_bandwidth_impl<F>(
        &self,
        user_label: &str,
        bucket_factory: F,
        bytes: u64,
        ...
        F: FnOnce() -> Arc<UserBucket>,
    {
        ...
        let user_bucket = bucket_factory();
    ...
    pub fn inc_user_connections(&self, user: &str) -> Result<usize> {
        self.inc_user_connections_arc(&Arc::<str>::from(user))
    }

    pub fn inc_user_connections_arc(&self, user: &Arc<str>) -> Result<usize> {
        let user_bucket = self.get_or_create_user_bucket_arc(user);
    ...
    pub fn dec_user_connections(&self, user: &str) -> usize {
        self.dec_user_connections_arc(&Arc::<str>::from(user))
    }

    pub fn dec_user_connections_arc(&self, user: &Arc<str>) -> usize {
        if let Some(user_bucket) = self.user_buckets.get(user.as_ref()) {
    ...
    pub fn get_user_connections(&self, user: &str) -> usize {
        self.get_user_connections_arc(&Arc::<str>::from(user))
    }

    pub fn get_user_connections_arc(&self, user: &Arc<str>) -> usize {
        self.user_buckets
            .get(user.as_ref())
    ...
    fn get_or_create_user_bucket_arc(&self, user: &Arc<str>) -> Arc<UserBucket> {
        if let Some(bucket) = self.user_buckets.get(user.as_ref()) {
            return bucket.clone();
        }

        let key = Arc::clone(user);
        self.user_buckets
            .entry(key)
            .or_insert_with(|| {
    ...
    fn get_or_create_user_bucket_str(&self, user: &str) -> Arc<UserBucket> {
        if let Some(bucket) = self.user_buckets.get(user) {
            return bucket.clone();
        }
    ...
    fn calculate_fair_shares(
        &self,
        active_users: &[(Arc<str>, Arc<UserBucket>, u64)],
    ) -> Vec<(Arc<str>, Arc<UserBucket>, u64)> {
        let mut allocations = Vec::new();
...
mod tests {
    ...
    async fn test_fair_sharing_calculation() {
        ...
        let active_users = vec![
            (Arc::<str>::from("alice"), bucket1, 500_000),
            (Arc::<str>::from("bob"), bucket2, 300_000),
            (Arc::<str>::from("charlie"), bucket3, 200_000),
        ];
    ...
    async fn test_rebalance_prefers_higher_demand_users() {
        ...
        let alice_bucket = htb.get_or_create_user_bucket_str("alice");
        let bob_bucket = htb.get_or_create_user_bucket_str("bob");
