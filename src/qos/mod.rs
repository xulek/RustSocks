...
impl QosEngine {
    ...
    pub async fn allocate_bandwidth_arc(&self, user: &Arc<str>, bytes: u64) -> Result<()> {
        match self {
            Self::None => Ok(()),
            Self::Htb(htb) => htb.allocate_bandwidth_arc(user, bytes).await,
        }
    }
    ...
    pub fn check_and_inc_connection_arc(
        &self,
        user: &Arc<str>,
        ...
        match self {
            ...
            Self::Htb(htb) => {
                ...
                let user_count = htb.get_user_connections_arc(user);
                if user_count >= limits.max_connections_per_user {
                    return Err(RustSocksError::Config(format!(
                        "User connection limit reached for '{}': {}/{}",
                        user, user_count, limits.max_connections_per_user
                ...
                let count = htb.inc_user_connections_arc(user)?;
                if count == 1 {
                    metrics::QosMetrics::user_activated();
                }
                Ok(count)
    ...
    pub fn dec_user_connection_arc(&self, user: &Arc<str>) {
        match self {
            Self::None => {}
            Self::Htb(htb) => {
                let remaining = htb.dec_user_connections_arc(user);
                if remaining == 0 {
                    metrics::QosMetrics::user_deactivated();
                }
            }
