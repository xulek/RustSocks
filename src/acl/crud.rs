...
/// Create a new user
pub fn create_user(config: &mut AclConfig, username: &str) -> Result<(), String> {
    ...
    info!(user = username, "Created new user");
    Ok(())
}

/// Delete a user
pub fn delete_user(config: &mut AclConfig, username: &str) -> Result<UserAcl, String> {
    let index = config
        .users
        .iter()
    ...
    let deleted_user = config.users.remove(index);

    info!(
        user = username,
        rule_count = deleted_user.rules.len(),
        group_count = deleted_user.groups.len(),
        "Deleted user"
    );
...
pub fn add_user_to_group(
    ...
    // Find or create user
    let user = if let Some(user) = config.users.iter_mut().find(|u| u.username == username) {
        user
    } else {
    ...
    if user.groups.iter().any(|g| g == group_name) {
        return Err(format!(
    ...
    info!(user = username, group = group_name, "Added user to group");
...
pub fn remove_user_from_group(
    ...
    let index = user
        ...
        .ok_or_else(|| format!("User '{}' is not in group '{}'", username, group_name))?;

    user.groups.remove(index);
