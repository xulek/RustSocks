            ...
            if parts.len() == 2 {
                if let (Ok(min), Ok(max)) = (parts[0].parse::<u32>(), parts[1].parse::<u32>()) {
                    ...
                    if range < 16384 {
                        ...
                        warn!("    Recommendation: Increase port range for high connection loads");
    ...
    match fs::read_to_string(path) {
        Ok(content) => {
            if let Ok(value) = content.trim().parse::<u32>() {
                if value > 30 {
                    warn!("⚠️  net.ipv4.tcp_fin_timeout is {} seconds (high)", value);
                    warn!(
...
fn check_tcp_tw_reuse() {
    ...
    match fs::read_to_string(path) {
        Ok(content) => {
            if let Ok(value) = content.trim().parse::<u32>() {
                if value != 2 {
                    warn!("⚠️  net.ipv4.tcp_tw_reuse is {} (suboptimal)", value);
                    warn!(
