...
// ============================================================================
// User CRUD & Group Assignment
// ============================================================================

/// POST /api/acl/users - Create new user
pub async fn create_user(
    State(state): State<ApiState>,
    Json(request): Json<crate::api::types::CreateUserRequest>,
) -> (StatusCode, Json<RuleOperationResponse>) {
    if state.acl_engine.is_none() {
        return (
            StatusCode::BAD_REQUEST,
    ...
    // Create user
    if let Err(e) = crud::create_user(&mut config, &request.username) {
        return (
            StatusCode::BAD_REQUEST,
            Json(RuleOperationResponse {
                success: false,
    ...
    info!(username = request.username, "Created new user via API");

    (
        StatusCode::OK,
        Json(RuleOperationResponse {
            success: true,
            message: format!("User '{}' created", request.username),
            rule: None,
            old_rule: None,
        }),
    )
...
/// DELETE /api/acl/users/{username} - Delete user
pub async fn delete_user(
    State(state): State<ApiState>,
    Path(username): Path<String>,
) -> (StatusCode, Json<crate::api::types::DeleteUserResponse>) {
    if state.acl_engine.is_none() {
        return (
            StatusCode::BAD_REQUEST,
            Json(crate::api::types::DeleteUserResponse {
                success: false,
                message: "ACL is not enabled".to_string(),
                deleted_user: None,
            }),
    ...
    // Delete user
    let deleted_user = match crud::delete_user(&mut config, &username) {
        Ok(u) => u,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    info!(username = username, "Deleted user via API");
    ...
    (
        ...
        Json(crate::api::types::DeleteUserResponse {
            ...
            deleted_user: Some(deleted_user),
        }),
    )
}

/// POST /api/acl/users/{username}/groups - Add user to group
pub async fn add_user_to_group(
    State(state): State<ApiState>,
    Path(username): Path<String>,
    Json(request): Json<crate::api::types::AddUserToGroupRequest>,
) -> (
    StatusCode,
    Json<crate::api::types::UserGroupOperationResponse>,
) {
    if state.acl_engine.is_none() {
        return (
            StatusCode::BAD_REQUEST,
    ...
    // Add user to group
    if let Err(e) = crud::add_user_to_group(&mut config, &username, &request.group_name) {
        return (
            StatusCode::BAD_REQUEST,
            Json(crate::api::types::UserGroupOperationResponse {
                success: false,
    ...
    (
        ...
        Json(crate::api::types::UserGroupOperationResponse {
            ...
            message: format!(
                "User '{}' added to group '{}'",
                username, request.group_name
            ),
        }),
    )
}

/// DELETE /api/acl/users/{username}/groups/{groupname} - Remove user from group
pub async fn remove_user_from_group(
    State(state): State<ApiState>,
    Path((username, group_name)): Path<(String, String)>,
) -> (
    StatusCode,
    Json<crate::api::types::UserGroupOperationResponse>,
) {
    ...
    if let Err(e) = crud::remove_user_from_group(&mut config, &username, &group_name) {
        return (
            StatusCode::NOT_FOUND,
            Json(crate::api::types::UserGroupOperationResponse {
                success: false,
    ...
    info!(
        username = username,
        group = group_name,
        "Removed user from group via API"
    );
    ...
    (
        ...
        Json(crate::api::types::UserGroupOperationResponse {
            success: true,
            message: format!("User '{}' removed from group '{}'", username, group_name),
        }),
    )
}
